...
class BurgersPINN(nn.Module):
    def __init__(self, layers: list, nu: float):
    ...
    def train_model(self, device: torch.device, epochs_adam: int = 2500, epochs_lbfgs: int = 100):
        ...
        optimizer_adam = torch.optim.Adam(self.parameters(), lr=0.003)
        N_pde = 8000
        ...
        for epoch in range(epochs_adam):
            ...
            if epoch % 500 == 0:
                print(f"Adam Epoch {epoch:4d} | Total Loss: {loss.item():.5f} | PDE Loss: {loss_pde.item():.5f}")
